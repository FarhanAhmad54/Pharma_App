...
def test_fefo_allocates_earliest_expiry_first() -> None:
    ...
    try:
        ...
        first = Batch(
            batch_number=f"INT-B1-{suffix}",
            product_id=product.id,
            warehouse_id=warehouse.id,
            manufacturing_date=manufacturing,
            expiry_date=manufacturing + timedelta(days=30),
        ...
        second = Batch(
            batch_number=f"INT-B2-{suffix}",
            product_id=product.id,
            warehouse_id=warehouse.id,
            manufacturing_date=manufacturing,
            expiry_date=manufacturing + timedelta(days=90),
            quantity_produced=Decimal("100"),
            quantity_available=Decimal("100"),
            status=BatchStatus.RELEASED,
            qc_status=QCStatus.RELEASED,
        ...
        first_stock = db.scalar(
            select(BatchInventory).where(BatchInventory.batch_id == first.id)
        )
        second_stock = db.scalar(
            select(BatchInventory).where(BatchInventory.batch_id == second.id)
        )
