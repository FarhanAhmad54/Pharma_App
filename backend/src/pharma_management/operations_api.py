...
def create_export(data: ExportCreate, db: Session = Depends(get_db), user: User = Depends(current_user)) -> dict[str, Any]:
    ...
    export = ExportOrder(**data.model_dump(), destination_country=data.destination_country.upper(), currency=data.currency.upper(), status="CONFIRMED", created_by=user.id)
    batch.quantity_available -= data.quantity
    db.add(export)
    db.add(InventoryMovement(product_id=data.product_id, batch_id=data.batch_id, warehouse_id=batch.warehouse_id, quantity=-data.quantity, movement_type=MovementType.SALE, reference_document=data.export_number, user_id=user.id, reason="Export allocation"))
    db.commit()
    db.refresh(export)
