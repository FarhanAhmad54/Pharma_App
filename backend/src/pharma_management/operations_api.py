...
def list_shipments(db: Session = Depends(get_db), _: User = Depends(current_user)) -> list[Shipment]:
    return list(db.scalars(select(Shipment).order_by(Shipment.created_at.desc()).limit(200)))


@router.post("/exports", status_code=201, dependencies=[Depends(require_roles(UserRole.ADMIN, UserRole.SALES_MANAGER))])
def create_export(data: ExportCreate, db: Session = Depends(get_db), user: User = Depends(current_user)) -> ExportOrder:
...
def inventory_report(db: Session = Depends(get_db), _: User = Depends(current_user)) -> list[dict]:
    rows = db.execute(
        select(
            ...
            func.sum(InventoryMovement.quantity).label("net_quantity"),
        ).group_by(InventoryMovement.product_id, InventoryMovement.warehouse_id)
    ).all()
    return [
        {"product_id": str(row.product_id), "warehouse_id": str(row.warehouse_id), "net_quantity": str(row.net_quantity or 0)}
        for row in rows
    ]
