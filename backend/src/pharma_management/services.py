...
from pharma_management.models import (
    AuditLog, Batch, BatchAllocation, BatchStatus, InventoryMovement, MovementType,
    OrderStatus, Product, ProductionOrder, ProductionStatus, QCRecord, QCStatus,
    SalesItem, SalesOrder, User, Warehouse,
)
from pharma_management.schemas import CompleteProductionRequest, ProductCreate, ProductUpdate, SaleCreate, TransferRequest


def audit(db: Session, user_id: UUID | None, action: str, entity: str, entity_id: str, old: str | None = None, new: str | None = None, reason: str | None = None) -> None:
    db.add(AuditLog(user_id=user_id, action=action, entity=entity, entity_id=entity_id, old_value=old, new_value=new, reason=reason))
...
def complete_production(db: Session, order: ProductionOrder, data: CompleteProductionRequest, user: User) -> Batch:
    ...
    if order.warehouse_id:
        db.add(InventoryMovement(product_id=order.product_id, batch_id=batch.id, warehouse_id=order.warehouse_id, quantity=data.actual_quantity, movement_type=MovementType.PRODUCTION, reference_document=order.order_number, user_id=user.id, reason="Production completion"))
...
def record_qc(db: Session, batch: Batch, data, user: User) -> Batch:
    ...
    db.add(QCRecord(batch_id=batch.id, reference_number=data.reference_number, test_date=data.test_date, tester_id=user.id, result=data.result, notes=data.notes))
...
def fefo_allocate(db: Session, product_id: UUID, warehouse_id: UUID, requested: Decimal, user: User, reference: str) -> list[BatchAllocation]:
    if requested <= 0:
        raise HTTPException(422, "Requested quantity must be positive")
    today = date.today()
    batches = db.scalars(select(Batch).where(Batch.product_id == product_id, Batch.warehouse_id == warehouse_id, Batch.status == BatchStatus.RELEASED, Batch.qc_status == QCStatus.RELEASED, Batch.expiry_date > today, Batch.quantity_available > 0).order_by(Batch.expiry_date.asc()).with_for_update()).all()
    remaining = requested
    ...
    for batch in batches:
        ...
        allocations.append(BatchAllocation(sales_item_id=UUID(int=0), batch_id=batch.id, quantity=qty))
        db.add(InventoryMovement(product_id=product_id, batch_id=batch.id, warehouse_id=warehouse_id, quantity=-qty, movement_type=MovementType.SALE, reference_document=reference, user_id=user.id, reason="FEFO allocation"))
        remaining -= qty
    if remaining > 0:
...
def create_sale(db: Session, data: SaleCreate, warehouse_id: UUID, user: User) -> SalesOrder:
    customer = db.get(__import__('pharma_management.models', fromlist=['Customer']).Customer, data.customer_id)
    ...
    order = SalesOrder(order_number=data.order_number, customer_id=data.customer_id, currency=data.currency.upper(), created_by=user.id)
    db.add(order)
    db.flush()
    total = Decimal("0")
    for item_data in data.items:
        ...
        item = SalesItem(sales_order_id=order.id, product_id=item_data.product_id, quantity=item_data.quantity, unit_price=item_data.unit_price)
        db.add(item)
        db.flush()
    ...
    audit(db, user.id, "SALE_ALLOCATED", "SalesOrder", str(order.id), new=str(total))
    db.commit()
    db.refresh(order)
    return order
...
def transfer_stock(db: Session, data: TransferRequest, user: User) -> None:
    ...
    db.add(InventoryMovement(product_id=data.product_id, batch_id=batch.id, warehouse_id=data.from_warehouse_id, quantity=-data.quantity, movement_type=MovementType.TRANSFER_OUT, user_id=user.id, reason=data.reason))
    batch.warehouse_id = data.to_warehouse_id
    db.add(InventoryMovement(product_id=data.product_id, batch_id=batch.id, warehouse_id=data.to_warehouse_id, quantity=data.quantity, movement_type=MovementType.TRANSFER_IN, user_id=user.id, reason=data.reason))
    batch.quantity_available += data.quantity
