...
import { CheckCircle2, Play, Square, X, Zap } from 'lucide-react'
...
export function ProductionControlPanel({ onRefresh, notice }: { onRefresh: () => Promise<void>; notice: Notice }) {
  ...
  async function load() {
    try { setOrders(await api.productionOrders()) } catch (error) { notice(error instanceof Error ? error.message : 'Unable to load production orders') }
  }
  useEffect(() => { void load() }, [])
  ...
  return <>
    <section className="panel module-table-panel" style={{ marginTop: 24 }}>
